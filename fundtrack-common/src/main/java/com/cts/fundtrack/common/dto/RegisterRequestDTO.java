...
public class RegisterRequestDTO {
    ...
    @Pattern(
            regexp = "^(?i)(ROLE_)?(ADMIN|FINANCE[_\\-\\s]?OFFICER|APPLICANT|APPROVER|REVIEWER|COMPLIANCE[_\\-\\s]?OFFICER)$",
            message = "Role must be one of: ADMIN, FINANCE_OFFICER, APPLICANT, APPROVER, REVIEWER, COMPLIANCE_OFFICER"
    )
    @Schema(description = "The authorization role for the new user",
            allowableValues = {"ADMIN", "FINANCE_OFFICER", "APPLICANT", "APPROVER", "REVIEWER", "COMPLIANCE_OFFICER"},
            example = "APPLICANT")
    private String role;
